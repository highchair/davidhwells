...
class Pyre_Latest_Blog_Media_Widget extends WP_Widget {
	...
	function Pyre_Latest_Blog_Media_Widget()
	{
		...
		$this->WP_Widget('pyre_homepage_media-widget', 'Homepage: Blog Posts', $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance)
	{
	 ...
	 ?>
		...
		<div class="<?php echo esc_attr(  $args['widget_id'] ); ?> homepage-widget-blog">
						...
						<div class="container-blog">
							<?php if(has_post_thumbnail()): ?>
								<div class="featured-blog-progression">
									<?php if( has_post_format( 'link' ) ): ?><a href="<?php echo esc_url( get_post_meta($post->ID, 'progression_external_link', true) );?>">
									...
									<div class="featured-blog-progression">
										<div class="flexslider gallery-progression">
								      	<ul class="slides">
											...
											<li><?php if( has_post_format( 'link' ) ): ?><a href="<?php echo esc_url( get_post_meta($post->ID, 'progression_external_link', true) );?>">
										<?php else: ?><a href="<?php the_permalink(); ?>"><?php endif; ?><img src="<?php echo esc_attr($thumbnail[0]);?>" alt="<?php echo esc_attr( $attachment->post_title );?>" /></a></li>
							...
							<?php endif; ?>
								...
								<div class="blog-post-text-bg">
									...
									<div class="entry-content">
										<?php the_content( __( 'Read more <span class="meta-nav">&rarr;</span>', 'progression' ) ); ?>
									</div><!-- .entry-content -->
									<?php endif; ?>
				
								</div><!-- close .blog-post-text -->
			
								<?php
								if ( is_sticky() && is_home() && ! is_paged() ) {
									printf( '<div class="sticky-post-pro">%s</div>', __( 'Featured Post', 'progression' ) );
								}
								?>
	...
	function form($instance)
	{
		...
		$defaults = array('title' => 'Latest News', 'summary_text' => '', 'categories' => 'all', 'posts' => 3, 'columns' => 3, 'link_text' => 'More News', 'link_link' => '#', 'link_icon' => '');
